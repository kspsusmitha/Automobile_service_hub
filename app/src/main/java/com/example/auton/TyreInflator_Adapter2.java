...
public class TyreInflator_Adapter2 extends RecyclerView.Adapter<TyreInflator_Adapter2.ViewHold> {
    private final Context context;
    private ArrayList<Accessories_ModelClass> dataList;


    public TyreInflator_Adapter2(Context fragment, ArrayList<Accessories_ModelClass> dataList) {
    ...
    @NonNull
    @Override
    public TyreInflator_Adapter2.ViewHold onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.horns_layout2, parent, false);
        return new TyreInflator_Adapter2.ViewHold(view);
    }

    public void onBindViewHolder(TyreInflator_Adapter2.ViewHold holder, int position) {
        Accessories_ModelClass ss = dataList.get(position);
        holder.manufacturer.setText(ss.getManufacturer());
        holder.desc.setText(ss.getModel() + " " + ss.getVoltage());
        ...
        holder.cardView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent i = new Intent(context.getApplicationContext(), fulldetails_TyreInflator.class);
