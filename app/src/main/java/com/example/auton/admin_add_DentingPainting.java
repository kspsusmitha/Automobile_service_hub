            ...
            if (servicenameStr.isEmpty() || wtsincludedStr.isEmpty() || priceStr.isEmpty()) {
                ...
                databaseReference.child("SERVICE_TYPE").child("DENTING_PAINITNG").addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        databaseReference.child("SERVICE_TYPE").child("DENTING_PAINITNG").child(servicenameStr).child("ServiceName").setValue(servicenameStr);
                        databaseReference.child("SERVICE_TYPE").child("DENTING_PAINITNG").child(servicenameStr).child("WtsIncluded").setValue(wtsincludedStr);
                        databaseReference.child("SERVICE_TYPE").child("DENTING_PAINITNG").child(servicenameStr).child("Price").setValue(priceStr);

                        Toast.makeText(admin_add_DentingPainting.this, "Value Entered Successfully", Toast.LENGTH_SHORT).show();
                    ...
                    public void onCancelled(@NonNull DatabaseError error) {
                        Toast.makeText(admin_add_DentingPainting.this, "error" + error.getMessage(), Toast.LENGTH_SHORT).show();
